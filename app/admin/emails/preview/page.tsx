...
// ============================================
// OseoVital Email Template — Navy + Gold Theme
// ============================================

const sampleOseoOrder = {
...
function generateOseoOrderConfirmation() {
  const { orderNumber, customerName, items, subtotal, shippingCost, total } = sampleOseoOrder
  const itemRows = items.map(i => `
    <tr>
  ...
  return oseoEmailWrapper(`
    <div style="text-align:center;margin-bottom:32px">
      ...
      <p style="font-size:13px;font-weight:600;margin:6px 0 0;letter-spacing:0.1em;text-transform:uppercase;color:#d4af37">Order #${orderNumber}</p>
    </div>
    <table style="width:100%;border-collapse:collapse;margin:24px 0">
      <thead><tr style="background:#f5f5f5"><th style="padding:10px 12px;text-align:left;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#999;font-weight:500">Product</th><th style="padding:10px 12px;text-align:center;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#999;font-weight:500">Qty</th><th style="padding:10px 12px;text-align:right;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#999;font-weight:500">Price</th></tr></thead>
      <tbody>${itemRows}</tbody>
    </table>
    <div style="background:#f5f5f5;padding:16px 20px;margin:24px 0">
      <table style="width:100%">
        <tr><td style="padding:4px 0;color:#666;font-size:13px">Subtotal</td><td style="padding:4px 0;color:#1a1a1a;font-size:13px;text-align:right">S$${subtotal.toFixed(2)}</td></tr>
        <tr><td style="padding:4px 0;color:#666;font-size:13px">Shipping</td><td style="padding:4px 0;color:#1a1a1a;font-size:13px;text-align:right">${shippingCost === 0 ? "FREE" : "S$" + shippingCost.toFixed(2)}</td></tr>
      </table>
      <div style="border-top:3px solid;border-image:linear-gradient(135deg,#b8860b,#d4af37,#f0c75e) 1;padding-top:12px;margin-top:8px"><table style="width:100%"><tr><td style="color:#1a1a1a;font-size:16px;font-weight:700">Total</td><td style="color:#00007b;font-size:16px;font-weight:700;text-align:right">S$${total.toFixed(2)}</td></tr></table></div>
    </div>
...
const templates = [
  { id: "order-confirmation", label: "Bloomie Order", generate: generateOrderConfirmation },
  ...
  { id: "oseo-order-confirmation", label: "OseoVital Order", generate: generateOseoOrderConfirmation },
]
